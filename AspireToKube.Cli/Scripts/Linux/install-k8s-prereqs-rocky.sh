...
# Install k9s
echo -e "${YELLOW}[3/6] Installing k9s...${NC}"
...
echo -e "${YELLOW}[4/6] Installing $K8S_TYPE (Kubernetes cluster)...${NC}"
...
echo -e "${YELLOW}[5/6] Installing Kubernetes Dashboard...${NC}"
...
echo -e "${YELLOW}[6/6] Lens installation (Kubernetes IDE)...${NC}"
    ...
    if [[ "$INSTALL_LENS" =~ ^[Yy]$ ]]; then
        if [ -f "/usr/local/bin/lens" ]; then
            ...
            if [ -f "OpenLens-${LENS_VERSION#v}.x86_64.AppImage" ]; then
                ...
                cat > /usr/share/applications/lens.desktop <<EOFLENS
[Desktop Entry]
Name=Lens
...
if [ "$K8S_TYPE" == "k3s" ]; then
    if command -v k3s &> /dev/null; then
        echo -e "${GREEN}[OK] k3s (Kubernetes cluster with kubectl)${NC}"
    else
        echo -e "${RED}[!!] k3s${NC}"
    fi
else
    if command -v minikube &> /dev/null; then
        echo -e "${GREEN}[OK] minikube (Kubernetes cluster with kubectl)${NC}"
    else

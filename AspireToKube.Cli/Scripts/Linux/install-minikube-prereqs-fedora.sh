...
# Prerequisites Installation Script for Fedora - Minikube Edition
...
if command -v docker &> /dev/null; then
    ...
    # Remove old versions and podman if they exist
    dnf remove -y docker \
                  docker-client \
...
if python3 -c "import cryptography" 2>/dev/null; then
    ...
    # On Fedora, we can use pip directly without --break-system-packages flag
...
if command -v getenforce &> /dev/null; then
    ...
    if [ "$SELINUX_STATUS" = "Enforcing" ]; then
        echo -e "${YELLOW}Note: SELinux is in enforcing mode${NC}"
        echo -e "${GRAY}If you encounter permission issues with Docker/Minikube:${NC}"
        echo -e "${GRAY}  - Check SELinux logs: ausearch -m avc -ts recent${NC}"
        echo -e "${GRAY}  - Or temporarily set to permissive: setenforce 0${NC}"
    fi
else
...
echo -e "   ${CYAN}minikube service <n>${NC} - Get URL for a service"
...
echo -e "${YELLOW}11. SELinux troubleshooting (if needed):${NC}"
echo -e "   ${CYAN}ausearch -m avc -ts recent${NC}    - Check SELinux denials"
echo -e "   ${CYAN}setenforce 0${NC}                  - Set to permissive mode (temporary)"
echo -e "   ${CYAN}setenforce 1${NC}                  - Set to enforcing mode"
echo ""

echo -e "${GREEN}Installation complete!${NC}"
echo -e "${GREEN}Your Minikube environment is ready for Aspire deployments!${NC}"
echo ""
echo -e "${YELLOW}Fedora Specific Notes:${NC}"
echo -e "  • SELinux is enabled by default - monitor for permission issues"
echo -e "  • Podman may have been removed to avoid conflicts with Docker"
echo -e "  • Docker repository uses Fedora-specific packages"
...
echo -e "  • Check logs: ${CYAN}journalctl -u docker${NC} or ${CYAN}minikube logs${NC}"
